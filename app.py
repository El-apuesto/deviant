...
class LLMClient:
    ...
    async def call_nvidia(self, messages, model=None, temperature=0.7, max_tokens=4000):
        async with self.semaphore:
            self._check_rate_limit()
            model = model or Config.NVIDIA_BUILDER_MODEL
    ...
    async def call_openrouter(self, messages, model=None, temperature=0.7, max_tokens=4000):
        async with self.semaphore:
            self._check_rate_limit()
            model = model or Config.RANKER_MODEL
            headers = {
                "Authorization": f"Bearer {self.openrouter_key}",
                "Content-Type": "application/json",
                "HTTP-Referer": "https://gardener-platform.local",
                "X-Title": "The Gardener"
            }
            payload = {"model": model, "messages": messages, "temperature": temperature, "max_tokens": max_tokens, "stream": False}
            try:
                async with httpx.AsyncClient(timeout=Config.REQUEST_TIMEOUT) as client:
                    response = await client.post(Config.OPENROUTER_API_URL, headers=headers, json=payload)
                    if response.status_code == 429 and model != Config.FALLBACK_RANKER:
                        return await self.call_openrouter(messages, Config.FALLBACK_RANKER, temperature, max_tokens)
                    response.raise_for_status()
                    data = response.json()
    ...
    async def call_ollama(self, messages, model=None, temperature=0.7, max_tokens=4000, cloud_host=None):
        """Call Ollama instance (local or cloud)."""
        async with self.semaphore:
            ...
            host = cloud_host or self.ollama_host
            try:
                async with httpx.AsyncClient(timeout=Config.REQUEST_TIMEOUT) as client:
    ...
    async def generate_code(self, prompt, model, system_prompt="", provider=None):
        messages = []
        if system_prompt: messages.append({"role": "system", "content": system_prompt})
        messages.append({"role": "user", "content": prompt})

        if provider is None:
            if "nvidia" in model or model == Config.NVIDIA_BUILDER_MODEL:
                provider = "nvidia"
            elif "/" not in model or "ollama" in model.lower():
                provider = "ollama"
            else:
                provider = "openrouter"

        if provider == "ollama":
            return await self.call_ollama(messages, model=model, temperature=0.3, max_tokens=6000)
        elif provider == "ollama-cloud":
            cloud_host = Config.OLLAMA_CLOUD_HOST
            if not cloud_host:
                return "ERROR: Ollama Cloud host not configured. Set it in Settings."
            return await self.call_ollama(messages, model=model, temperature=0.3, max_tokens=6000, cloud_host=cloud_host)
        elif provider == "nvidia":
            return await self.call_nvidia(messages, model=model, temperature=0.3, max_tokens=6000)
        else:
            return await self.call_openrouter(messages, model=model, temperature=0.3, max_tokens=6000)

    async def test_connection(self, provider, model=None):
        """Test if a provider is reachable."""
        test_messages = [{"role": "user", "content": "Say 'OK'"}]
        if provider == "ollama":
            result = await self.call_ollama(test_messages, model=model or "llama3.2")
        elif provider == "ollama-cloud":
            cloud_host = Config.OLLAMA_CLOUD_HOST
            if not cloud_host:
                return False, "Ollama Cloud host not configured. Enter the URL in Settings."
            result = await self.call_ollama(test_messages, model=model or "llama3.2", cloud_host=cloud_host)
        elif provider == "nvidia":
            result = await self.call_nvidia(test_messages, model=model or Config.NVIDIA_BUILDER_MODEL)
        elif provider == "openrouter":
            result = await self.call_openrouter(test_messages, model=model or Config.RANKER_MODEL)
        else:
...
else showError(result.error||'Build failed');
}catch(err){showError('Network error: '+err.message);}
});
function startPolling(){if(pollInterval)clearInterval(pollInterval);pollInterval=setInterval(async()=>{try{const res=await fetch('/api/progress/'+buildId);const p=await res.json();updateProgress(p);if(p.status==='complete'||p.status==='failed'){clearInterval(pollInterval);if(p.status==='complete')loadResults(buildId);}}catch(err){console.error(err);}},2000);}
...
async function loadResults(id){try{const res=await fetch('/api/results/'+id);const r=await res.json();if(r.status==='success')displayResults(r);else showError(r.error);}catch(err){showError('Failed to load results');}}
...
function showError(m){document.getElementById('progressMessage').textContent='ERROR: '+m;document.getElementById('progressFill').style.width='100%';document.getElementById('progressFill').style.background='var(--error)';}
...
async def get_results(build_id: str):
    results = orchestrator.results.get(build_id)
    if not results:
...
async def get_leaderboard(timeframe: str = "all_time", code_type: str = None, sort_by: str = "score", limit: int = 50):
    ...
    return {"entries": [e.dict() for e in entries], "stats": stats}
